...
export const SubSequencePanel: React.FC<SubSequencePanelProps> = ({
  ...
  const { state, validateSequence } = useMscEditor();
  ...
  const subSequences = useMemo(() => {
    ...
    return sequence.subSequences.map((sub, _index) => ({
      id: sub.id,
      name: sub.name,
      sequence: sub,
  ...
  const handleSaveSubSequence = (_updatedSub: MscSequence) => {
        ...
        <Stack>
          ...
          <ScrollArea h={400}>
            <Paper withBorder p="md" mt="md">
              ...
              <Stack gap="sm">
                {editingSubSequence.messages.map((message, _index) => (
      ...
      <ScrollArea h={height - 120} styles={(theme) => ({
        ...
        <Stack gap="md">
          {subSequences.length === 0 ? (
            ...
            subSequences.map((item, index) => renderSubSequenceItem(item as unknown as SubSequenceItem, index))
          )}
        </Stack>
      </ScrollArea>
