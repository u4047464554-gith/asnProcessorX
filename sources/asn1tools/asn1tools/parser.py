...
def convert_parameterized_object_class_assignment(_s, _l, tokens):
    tokens = tokens.asList()
    name = tokens[0]
    parameter_spec = tokens[1]
    rhs = tokens[2]
    rhs_details = tokens[3]

    if rhs == 'CLASS':
        members = []

        for member in rhs_details:
            field_name = member[0]

            if not isinstance(field_name, str) or len(field_name) < 2 or not field_name.startswith('&'):
                raise ParseSyntaxException(_s, _l, "Invalid object class member")

            if field_name[1].islower():
            ...
            converted_member['name'] = field_name

            members.append(converted_member)

        converted_type = {'members': members}

        if isinstance(parameter_spec, dict):
            converted_type.update(parameter_spec)
    else:
        converted_type = {'type': rhs}

        if rhs_details:
            converted_type.update(rhs_details)

    return ('parameterized-object-class-assignment', name, converted_type)
...
def convert_assignment_list(_s, _l, tokens):
    ...
    object_classes = {}
    object_sets = {}
    pending_object_class_invocations = []
    ...
    for kind, name, value in tokens:
        if kind == 'parameterized-object-set-assignment':
            ...
            if 'members' in value:
                ...
                pending_object_class_invocations.append((name, value))
    ...
    for name, value in pending_object_class_invocations:
        base = value.get('type')

        if base in object_classes:
            object_classes[name] = value
            continue

        if base in types:
            type_value = {'type': base}
            actual_parameters = value.get('actual-parameters')

            if actual_parameters:
                type_value['actual-parameters'] = actual_parameters
